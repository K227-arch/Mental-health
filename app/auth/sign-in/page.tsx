...
export default function SignInPage() {
  ...
  const handleOAuth = async (provider: string) => {
    ...
    // Mark this as a SIGN-IN so the callback can refuse brand-new accounts.
    document.cookie = `insforge_intent=signin; path=/; max-age=600; SameSite=Lax`;
    ...
    <div className="min-h-screen flex bg-surface relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 py-10 relative">
        ...
        <div className="bg-surface-container-lowest border border-outline-variant/40 rounded-3xl p-6 sm:p-7 shadow-lg shadow-primary/5">
          {/* Title */}
          <h1 className="text-lg font-bold text-on-surface mb-0.5">{t("auth.signin.title")}</h1>
          <p className="text-xs text-on-surface-variant mb-5">{t("auth.signin.subtitle")}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button type="button" onClick={handleResetPassword} disabled={resetLoading} className="text-xs text-primary font-medium hover:underline disabled:opacity-50">
              ...
              <div className="flex items-start gap-2 p-3 bg-error-container/80 text-on-error-container text-sm rounded-xl animate-fade-in">
                ...
                <div>
                  ...
                  {error.includes("sign up first") && (
                    <a href={`/auth/sign-up?role=${role}`} className="block mt-1 text-primary font-semibold hover:underline text-xs">
                      Create an account →
                    </a>
                  )}
                </div>
        ...
        <p className="text-center text-sm text-on-surface-variant mt-6">
          ...
          <a href={`/auth/sign-up?role=${role}`} className="text-primary font-semibold hover:underline">
